...
public class CustomerWatch extends ListenableModel implements CrudReady<CustomerWatch, CustomerWatch>, Searchable {
    ...
    public static List<CustomerWatch> findAllByWatchmaker() {
    	return find.fetch("customer").fetch("managedBy").where().eq("service_price_accepted", true).eq("serviceNeeded", true).ne("status", "BACK_TO_CUSTOMER").isNotNull("managedBy").ne("service_status", 100).orderBy("managedBy desc, id DESC").findList();
    }
    
    public static List<CustomerWatch> findFinishedByWatchmaker() {
    	return find.fetch("customer").fetch("managedBy").where().isNotNull("managedBy").eq("service_status", 100).orderBy("managedBy desc, id DESC").findList();
    }
    ...
    public static List<CustomerWatch> findAllToTestOrderedByID() {
        return find.fetch("customer").where().ne("status", "BACK_TO_CUSTOMER").eq("serviceNeeded", true).eq("service_status", 100).orderBy("id desc").findList();
    }
    
    public static List<CustomerWatch> findAllEmergencyOrderedByID() {
        return find.fetch("customer").where().eq("service_price_accepted", true).eq("serviceNeeded", true).ne("status", "BACK_TO_CUSTOMER").ne("service_status", 100).orderBy("id desc").findList();
    }
    ...
    public JsonNode toJson() {
        ...
        json.put("id", id)
        	...
        	.put("firstKnownDate", DateHelper.asShortDate(getFirstKnownDate()))
        	.put("firstKnownDate", DateHelper.asShortDate(getFirstKnownDate()))
        	.put("hasCalledForDelay", isCustomerHasCalledForDelay())
        	.put("hasConstraint", isCustomerHasConstraint())
