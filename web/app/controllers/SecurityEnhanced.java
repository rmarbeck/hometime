...
public class SecurityEnhanced {
    ...
    public static class AuthenticatedAction extends Action<Authenticated> {
		...
		public F.Promise<Result> call(Context ctx) {
            try {
                Authenticator authenticator = configuration.value().getDeclaredConstructor().newInstance();
                String username = authenticator.getUsername(ctx);
                if(username == null || !authenticator.isRoleAuthorized(username, configuration.rolesAuthorized())) {
    ...
    public static class Authenticator extends Results {
        ...
        public static boolean isRoleAuthorized(String username, User.Role[] rolesAuthorized) {
        	return SecurityHelper.isUserAuthorized(username, rolesAuthorized);
        }
    }
